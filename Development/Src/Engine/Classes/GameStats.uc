...
{
	if ( (Who.bBot && !bShowBots) || Who.bOnlySpectator )			// Just to be totally safe Spectators sends nothing
		return;
...
{
	local string out;
	if ( (Victim.bBot && !bShowBots) || Killer.bOnlySpectator || Victim.bOnlySpectator ) // Spectator might suicide
		return;
...
function SpecialEvent(PlayerReplicationInfo Who, string Desc)
{
	...
	if (Who != None)
	{
		if ( (Who.bBot && !bShowBots) || Who.bOnlySpectator )		// Avoid spectator suicide on console "type_kill"
			return;
...
function GameEvent(string GEvent, string Desc, PlayerReplicationInfo Who)
{
	...
	if (Who != None)
	{
		if ( (Who.bBot && !bShowBots) || Who.bOnlySpectator )		// Specator could cause NameChange, TeamChange! No longer.
			return;
		out = ""$Controller(Who.Owner).PlayerNum;
	}
	else
