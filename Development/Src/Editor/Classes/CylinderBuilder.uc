...
var() float Z, OuterRadius, InnerRadius;
var() int Sides;
var() name GroupName;
var() bool AlignToSide, Hollow;

function BuildCylinder( int Direction, bool AlignToSide, int Sides, float Z, float Radius )
{
	local int n,i,j,Ofs;
	n = GetVertexCount();
	if( AlignToSide )
	...
	for( i=0; i<Sides; i++ )
		for( j=-1; j<2; j+=2 )
			Vertex3f( Radius*sin((2*i+Ofs)*pi/Sides), Radius*cos((2*i+Ofs)*pi/Sides), j*Z/2 );
