...
public:
	...
	//
	// Load the actor from a file : mesh plus reference skeleton.
	//
	...
	int LoadActor(FastFileClass &InFile)
	{
		...
		//if( ! _tcscmp(ChunkHdr.ChunkID,"ACTRHEAD") )
		//	PopupBox("Error: PSK file reading: ACTRHEAD chunk not found.");
		...
		SkinData.RawWeights.AddExactZeroed(ChunkHdr.DataCount);
	...
	int LoadAnimation(FastFileClass &InFile)
	{
		...
		// SKIP the bones - relying on our scene to have consistent (number of) bones... #TODO: add 
		RefSkeletonBones.Empty();
