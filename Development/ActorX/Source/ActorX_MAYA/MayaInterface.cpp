...
//
void RetrieveTrafoForJoint( MObject& ThisJoint, MObject& ThisParent, FMatrix& ThisMatrix, FQuat& ThisQuat, INT RootCheck, FVector& BoneScale )
{
...
int SceneIFC::DigestAnim(class VActor *Thing,char *SequenceName,char *RangeString)
{
	...
	_tcscpy(Thing->RawAnimName,CleanString( SequenceName ));
	...
	for( INT t=0; t<FrameList.GetTotal(); t++)		
	{
		...
		for (int i = 0; i<SerialTree.Num(); i++) // i = RootBoneIndex...
		{
			...
			{			
				...
				RetrieveTrafoForJoint( JointObject, ParentObject, LocalMatrix, LocalQuat, RootCheck, BoneScale );
...
int SceneIFC::WriteVertexAnims(VActor *Thing, char* DestFileName, char* RangeString )
{
	...
	if( VAFaces.Num() && (OurScene.DoAppendVertex == 0) )
	{
		...
		sprintf(OutPath,"%s\\%s_d.3d",(char*)to_path,(char*)DestFileName);		
	...
	if( VARawVerts.Num() )
	{
		...
		FastFileClass OutFile;
		char OutPath[MAX_PATH];
		sprintf(OutPath,"%s\\%s_a.3d",(char*)to_path,(char*)DestFileName);				
		//PopupBox("OutPath for _a file: %s",OutPath); 
		...
		else
		{
			...
			// Need to explicitly move file ptr to end.
			OutFile.Seek(OutFile.GetSize()); 
		}
		...
		{			
			...
			*/
			...
			if( 1 ) // TriFace->flags & HAS_TVERTS )
			{
				...
				stat = MeshFunction.getPolygonUV( PolyIndex,VertIdx[0],U,V,&UVSet1  );
				//if ( stat != MS::kSuccess )
				...
				stat = MeshFunction.getPolygonUV( PolyIndex,VertIdx[1],U,V,&UVSet2  );
				//if ( stat != MS::kSuccess )
				//	DLog.Logf(" Invalid UV retrieval ");
				Wedge1.U =  U;
				Wedge1.V =  1.0f - V; 
				...
				stat = MeshFunction.getPolygonUV( PolyIndex,VertIdx[2],U,V,&UVSet3  );
				//if ( stat != MS::kSuccess )
				//	DLog.Logf(" Invalid UV retrieval ");
				Wedge2.U =  U;
				Wedge2.V =  1.0f - V;
...
//
void GetBitmapName(TCHAR* Name, TCHAR* Path, MObject& ThisShader )
{
	...
	if( strlen( TextureSourceString ) >0 )
	{
		...
		sprintf( Name,"%s.%s",BmpFilename,BmpExtension);
		strcpysafe(Path,CleanString(BmpPathname),MAX_PATH);	
	}
	else
	{
		sprintf( Name,"unknown_bitmap.bmp");
	}
}
...
int	SceneIFC::FixMaterials(VActor *Thing )
{
	...
	for( DWORD i=0; i< AxShaders.length(); i++ )
	{
		...
		GetBitmapName( TextureSource.RawBitmapName, TextureSource.RawBitmapPath, AxShaders[i] );		
	...
	for (int i = 0; i<SerialTree.Num(); i++) // i = RootBoneIndex...
	{
		...
		{ 
			...
			*/
			...
			//NOTE: this is an irresponsably hardcoded scene root assumption ?
			MObject ParentObject = AxSceneObjects[ (int)SerialTree[5].node ];  
...
MStatus AXWriteSequence(BOOL bUseSourceOutPath)
{
	...
	if( TempActor.Animations.Num() == 1 )
	{
		...
		TempActor.OutAnims[0].AnimInfo = TempActor.Animations[0].AnimInfo;
		for( INT j=0; j<TempActor.Animations[0].KeyTrack.Num(); j++ )
		{								
			TempActor.OutAnims[0].KeyTrack.AddItem( TempActor.Animations[0].KeyTrack[j] );
		}	
	...
	sprintf(OutPath,"%s\\%s.%s", OutFolder, SceneName, to_ext);
...
MStatus AXWritePoses()
{
	...
	for( INT t=(INT)TimeStart; t<=(INT)TimeEnd; t++)
	{
		...
		sprintf(SingleFrameString,"%i",t);
		sprintf(NumberedSequenceName,"%s%i",SceneName,t);
		...
		char to_ext[32];
		_tcscpy(to_ext, ("PSA"));
		sprintf( OutPath,"%s\\%s%i.%s",(char*)to_path,(char*)SceneName,FrameSweepCounter,to_ext);					
...
DeclareCommandClass( ActorXTool9 );
...
MStatus initializePlugin( MObject _obj )						 
{
	...
	MFnPlugin	plugin( _obj, "Epic Games, Inc.", "2.37  ", "Any"  ); // VERSION
	MStatus		stat;
	...
	stat = plugin.registerCommand( "axbrush", ActorXTool9::creator );
	if (stat != MS::kSuccess)												 
		stat.perror("registerCommand error: axbrush");	
	...
	stat = plugin.registerCommand( "axexecute", ActorXTool11::creator );
	if (stat != MS::kSuccess)
...
//"AXBrush"
MStatus ActorXTool9::doIt( const MArgList& args )
{
	ClearScenePointers(); 
	MStatus stat = MS::kSuccess;
	HWND hWnd = GetActiveWindow();
	...
	setResult( "command axbrush executed!\n" );
	return stat;
}
...
INT ParseSpecifier(const MArgList& args, const char* specifier )
{
	...
	{
		char TempArg[1024];
		strcpy( TempArg, args.asString(i).asChar() );
...
//
UBOOL ParseCommand( const MArgList& args, const char* specifier, char* Output )
{
	...
	char TempOut[1024];
	TempOut[0]=0;
	strcpy( TempOut, args.asString(SpecIdx+1).asChar() );
...
// Find if specifier is in the argument list; if so, return the next argument .
//
UBOOL ParseCommandKeepCase( const MArgList& args, const char* specifier, char* Output )
{
	INT SpecIdx = ParseSpecifier( args, specifier );
...
UBOOL ParseMultiArgCommand( const MArgList& args, const char* specifier, char* Output )
{
	INT SpecIdx = ParseSpecifier( args, specifier );
...
MStatus ActorXTool11::doIt( const MArgList& args )
{
	...
	UBOOL DoDigestAnim = ParseCommand( args, "-sequence", ExSequenceName );	
	...
	UBOOL DoAppendAnim = ParseSwitch(args,"-appendfile");
	UBOOL DoSaveAnim = ParseSwitch(args,"-saveanim");

	UBOOL BackupDoAutoTriangles = OurScene.DoAutoTriangles;
	UBOOL BackupDoUnSmooth =  OurScene.DoUnSmooth;

	OurScene.DoAutoTriangles = ParseSwitch(args,"-autotri");  // On by default, though ?
	OurScene.DoUnSmooth = ParseSwitch(args,"-unsmooth"); 
	...
	{
		_tcscpy(to_path,ExPathName); 
		_tcscpy(LogPath,ExPathName);	
	}
	...
	if( DoSkin )
	{
		...
		{
			_tcscpy(to_skinfile, ExSkinFileName );
		}
	...
	{		
		...
		if( strlen( ExSequenceName ) )
		{
			...
			_tcscpy( newsequencename, ExSequenceName );
			if( ForcedRate !=0.0f )
			{
...
MStatus ActorXTool12::doIt( const MArgList& args )
{
	...
	GetLoadName( hWnd, PSKpath, to_path, filterList);
	//GetNameFromPath(to_pskfile,newname, MAX_PATH );	
	
	_tcscpy( SAVEDpath, PSKpath );
...
static BOOL CALLBACK PanelTwoDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{	
	switch (msg) 
	{
		case WM_INITDIALOG:
...
static BOOL CALLBACK VertExpDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{	
	switch (msg) 
	{
		case WM_INITDIALOG:
...
void MeshProcessor::fillSmoothFaces( INT polyid,  MFnMesh& fnMesh )
{		
...
//
//
//  Smoothing group extraction algorithm from the Maya SDK's  MeshProcessor sources.
//  In:  MeshProcessor.buildEdgeTable( DagPath of  a mesh )
//  Out: MeshProcessor.PolySmoothingGroups -> a smoothing group (NUMERICAL, not yet a shifted bit- ) for each face..
//  This will only effectively flood-fill areas bounded by sharp edges with different smoothing groups.  
//  Any single edges within such areas are lost.
//
void MeshProcessor::buildEdgeTable( MDagPath& mesh )
{
	...
	// edgeTable.Empty();
	// edgeTable.AddZeroed( edgeTableSize ); //Start with as many edges as vertices.

    // Add entries, for each edge, to the lookup table
    MItMeshEdge eIt( mesh );
    for ( ; !eIt.isDone(); eIt.next() )
    {
        bool smooth = eIt.isSmooth();  // Crucial smoothnes setting source.
    ...
    for ( ; !pIt.isDone(); pIt.next() )
    {
        ...
        for ( int v=0; v<pvc; v++ )
        {
			// Circle around polygons assigning the edge IDs 
			// to edgeinfo's 
			// 
            int a = pIt.vertexIndex( v );
	...
	// table. Note: we have to traverse ALL polygons to handle the case
	...
	// Deallocate the multi-level-TArray VertEdgePools properly.
...
UBOOL MeshProcessor::smoothingAlgorithm( INT polyId, MFnMesh& fnMesh )
{
    MIntArray vertexList; 
    ...
    for ( int vid=0; vid<vcount;vid++ ) 
	{
        ...
        if ( Edge ) 
		{
            ...
            //       halfEdge polygons get a smoothing group of NO_SMOOTHING_GROUP.
            if ( NO_SMOOTHING_GROUP != Edge->polyIds[1] ) 
			{ 
				// Edge not a border                
                ...
                // Find the adjacent poly id, other side of the smooth edge.          
                int adjPoly = Edge->polyIds[0];
                if ( adjPoly == polyId ) 
				{
                    adjPoly = Edge->polyIds[1];
                }                             
				...
				{
					// recurse - find the smoothing group for an unprocessed adjPoly.
                    smoothingAlgorithm( adjPoly, fnMesh );
                }
                else 
				if ( (DWORD)PolySmoothingGroups[adjPoly] != (DWORD)currSmoothingGroup ) 
				{
					DLog.Logf("Warning: smoothing group problem at polyon [%i]", adjPoly);                    
                }
            }
        }
    }
