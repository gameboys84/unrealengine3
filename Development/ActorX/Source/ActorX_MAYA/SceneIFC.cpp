...
int	SceneIFC::DoUnSmoothVerts(VActor *Thing, INT DoTangentVectorSeams  )
{
	...
	for( INT p=0; p< Fans.Num(); p++) // The fan of faces for each 3d point 'p'.
	{
		...
		if( Fans[p].FaceRecord.Num() > 0 )
		{
			...
			//PopupBox(" TotalSmoothMatches %i CxChex %i  Groups for 3d point %i :  [ %i]  total faces for point are %i ",TotalSmoothMatches, TotalConnexChex  ,p, FaceSets.Num(),  Fans[p].FaceRecord.Num() )  ; //#SKEL 4 and 5 output, instead of 5 and 6 ?!?!
