...
static BOOL CALLBACK BoneTreeDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
...
BOOL CALLBACK ActorManagerDlgProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	*/		
					...
					{							
						...
						case LBN_SELCHANGE:						
						{
							...
							if( InSelBufferNum )
							{
								...
								_SetCheckBox( hWnd, IDC_CHECKSCALE, TempActor.Animations[InSelector].AnimInfo.RootInclude );
								*/							
								...
								//////////////////////////////
								UBOOL SameCheckScales = true;								
								{ 
									UBOOL Inf1,Inf2;
									for( INT c=0; c<InSelBufferNum; c++)
									{
										INT AnimIdx=InSelBuffer[c];									
										Inf1 = TempActor.Animations[AnimIdx].AnimInfo.ScaleInclude;									
										if( c>0 ) 
										{
											if( Inf1 != Inf2 )
											{
												SameCheckScales = false;
											}
										}
										Inf2 = Inf1;
									}
								...
								if( SameCheckScales )
									_SetCheckBox( hWnd, IDC_CHECKSCALE, TempActor.Animations[InSelBuffer[0]].AnimInfo.ScaleInclude );
								else
									_SetCheckBox( hWnd, IDC_CHECKSCALE, 0 );
								///////////////////////
							}
							else
							{ 
	...
	if( UpdateAnimParams )
	{
		...
		if( UpdateCheckScale )
		{
			UBOOL ScaleCheck = _GetCheckBox( hWnd, IDC_CHECKSCALE );
			for( INT c=0; c<InSelBufferNum; c++)
			{
				INT AnimIdx=InSelBuffer[c];
				TempActor.Animations[AnimIdx].AnimInfo.ScaleInclude = ScaleCheck;
			}
		}
	}
	...
	switch (msg) 
	{
		case WM_INITDIALOG:
			//CenterWindow(hWnd, GetParent(hWnd)); 
			{
				...
				PrintWindowNum(hWnd,IDC_PHYMESHES,  OurScene.PhysiqueNodes);
