...
template <class Type> class TArray {
	public:
		...
		int		Num() const;
                ...
                void	Append(TArray<Type> const& other, bool memcopysafe = true);
...
template <class Type>
void TArray<Type>::AddItem(Type t)
{	
	INT NewIdx = ArrayNum;
	Add(1);
	Data[NewIdx] = t;	
...
// APPEND - PLE-only #DEBUG - check for safety...
template <class Type>
void	TArray<Type>::Append(TArray<Type> const& other, bool memcopysafe)
{
	if (other.Num() == 0)
		return;
	if (&other == this) {
		TArray<Type> copy(other);
		Append(copy,memcopysafe);
		return;
	}
	...
	Add( other.Num() );
	if (memcopysafe) {
		memcpy( Data+oldsize, other.Data, other.Num() * sizeof(Type) );
	} else {
		for (int i = 0; i < other.Num(); i++) {
			Data[oldsize+i] = other.Data[i];
		}
	}
}
