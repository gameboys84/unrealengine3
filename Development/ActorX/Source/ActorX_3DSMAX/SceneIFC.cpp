/************************************************************************** 
	...
	Copyright (c) 2000,2001,2002,2003,2004  Epic Games, Inc. All Rights Reserved.
...
int	SceneIFC::DoUnSmoothVerts(VActor *Thing, INT DoTangentVectorSeams  )
{
	...
	if( DoTangentVectorSeams )
	{
		...
		for( INT FaceIndex=0; FaceIndex< Thing->SkinData.Faces.Num(); FaceIndex++)
		{
			...
			FLOAT T1U= Thing->SkinData.Wedges[ Thing->SkinData.Faces[FaceIndex].WedgeIndex[0] ].U;
			FLOAT T1V= Thing->SkinData.Wedges[ Thing->SkinData.Faces[FaceIndex].WedgeIndex[0] ].V;

			FLOAT T2U= Thing->SkinData.Wedges[ Thing->SkinData.Faces[FaceIndex].WedgeIndex[1] ].U;
			FLOAT T2V= Thing->SkinData.Wedges[ Thing->SkinData.Faces[FaceIndex].WedgeIndex[1] ].V;

			FLOAT T3U= Thing->SkinData.Wedges[ Thing->SkinData.Faces[FaceIndex].WedgeIndex[2] ].U;
			FLOAT T3V= Thing->SkinData.Wedges[ Thing->SkinData.Faces[FaceIndex].WedgeIndex[2] ].V;
	...
	for( INT p=0; p< Fans.Num(); p++) // The fan of faces for each 3d point 'p'.
	{
		...
		{		
			...
			//PopupBox(" TotalSmoothMatches %i CxChex %i  Groups for 3d point %i :  [ %i]  total faces for point are %i ",TotalSmoothMatches, TotalConnexChex  ,p, FaceSets.Num(),  Fans[p].FaceRecord.Num() )  ;
