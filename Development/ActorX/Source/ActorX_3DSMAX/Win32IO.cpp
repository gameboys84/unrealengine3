...
int GetNameFromPath(char* Dest, const char* Src, int MaxChars )
{
	...
	char* ChStart  = strrchr(Src,char(92));    // \ 92 
	char* ChStart2 = strrchr(Src,char(47));    // / 47 
	char* ChEnd    = strrchr(Src,char(46));    // . 46
...
{
	char* ChEnd    = strrchr(Src,char(0));   
	char* ChStart  = strrchr(Src,char(46));   
	if( !ChEnd )
	{
		Dest[0] = 0;
		return 0;
	...
	*/
	...
	while (1) {
		...
		if (GetSaveFileName(&ofn)) {
			...
			int FileNameLength = _tcslen(ofn.lpstrFile);
			if (FileNameLength == 0)
				return;
