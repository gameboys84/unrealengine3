...
static MaxInterfaceInitialized = 0;
...
//
TriObject* GetTriObjectFromNode(AXNode *node, TimeValue t, int &deleteIt)
{
...
int SceneIFC::DigestSkeleton(VActor *Thing)
{
	...
	{	
		...
		{	
			...
			SkelLog.Logf("Node: %4i %-23s Dummy: %2i PointHelper %2i Bone: %2i Skin: %2i PhyLinks: %4i NodeID %8i Tex: %i Culld: %2i Parent:%2i\n",i,
				NodeName,
				SerialTree[i].IsDummy,
				SerialTree[i].IsPointHelper,
				SerialTree[i].IsBone,
...
int	SceneIFC::DigestSkin(VActor *Thing )
{
	...
	if ( DoQuadTex )
	{
		...
		for( i=0; i<MAXSKININDEX; i++)
			SkinIndex[i]=0;

		// Gather material indices:
		for( i=0; i<Thing->SkinData.Wedges.Num(); i++)
		{
		...
		for(  i=0; i<Thing->SkinData.Wedges.Num(); i++)
		{
			...
			if( Skin == MainMaterial )
			{
				...
				FLOAT Uf = Thing->SkinData.Wedges[i].U * 2.f;
				FLOAT Vf = Thing->SkinData.Wedges[i].V * 2.f;
				...
				Thing->SkinData.Wedges[i].U  = Uf;
				Thing->SkinData.Wedges[i].V  = Vf;
...
int	SceneIFC::ProcessMesh(AXNode* SkinNode, int TreeIndex, VActor *Thing, VSkin& LocalSkin, INT SmoothSkin, SkinInf* SkinHandle )
{
	...
	for (int TriIndex = 0; TriIndex < NumFaces; TriIndex++)
	{
		...
		if( TotalTVNum > 0 )   // (TriFace->flags & HAS_TVERTS)  
		{
			...
			Wedge0.U =  UV0.x;
			Wedge0.V =  1.0f-UV0.y; //#debug flip Y always ?
			Wedge1.U =  UV1.x;
			Wedge1.V =  1.0f-UV1.y;
			Wedge2.U =  UV2.x;
			Wedge2.V =  1.0f-UV2.y;
